...
class UpdateService {
  ...
  Future<UpdateInfo> checkForUpdate({bool manual = false}) async {
    try {
      if (!manual) {
        ...
        final suppressDate = prefs.getString('suppress_update_date');
      ...
      final response = await http.get(
        ...
        headers: {
          ...
          'User-Agent': 'HotelPOS-Admin-Updater',
        },
      ...
      for (final release in releases) {
        ...
        for (final asset in assets) {
          if (asset is Map) {
            ...
            if (name.toLowerCase().contains('admin') && name.toLowerCase().endsWith('.apk')) {
  ...
  Future<void> downloadAndInstallApk(
    ...
    try {
      ...
      final apkFile = File('${tempDir.path}/hotel_admin_update.apk');
